...
Raytracer *cornell_box_scene() {
  ...
  Hitable *b1 =
      new RotateY(new Box(Vect(130, 0, 65), Vect(295, 165, 230), white), -18);
  list[i++] =
      new ConstantVolume(b1, 0.0004, new ConstantTexture(Vect(1.0, 1.0, 1.0)));
  list[i++] =
      new RotateY(new Box(Vect(265, 0, 295), Vect(430, 330, 460), white), 15);
...
Raytracer *get_scene(double tmin, double tmax) {
  ...
  Texture *checker =
      new CheckerTexture(new ConstantTexture(Vect(0.2, 0.3, 0.1)),
                         new ConstantTexture(Vect(0.9, 0.9, 0.9)));
  MovingSphere *mv = new MovingSphere(
      Vect(0, 1.0, -1), 0.5,
      new Lambertian(new ConstantTexture(Vect(0.1, 0.2, 0.5))));
  objects[0] = mv;
  objects[1] = new Sphere(Vect(0, -100.5, -1), 100, new Lambertian(checker));
  objects[2] =
      new Sphere(Vect(1, 0.0, -1.0), 0.5, new Metal(Vect(0.8, 0.6, 0.2)));
...
Hitable *getLightRectangle(double x0, double x1, double y0, double y1, double k,
                           Vect color) {
...
Raytracer *get_random_scene() {
  ...
  list[0] =
      new Sphere(Vect(0, -1000, 0), 1000, new Lambertian(Vect(0.5, 0.5, 0.5)));
  int i = 1;
  for (int a = -11; a < 11; a++) {
    for (int b = -11; b < 11; b++) {
      float choose_mat = drand48();
      Vect center(a + 0.9 * drand48(), 0.2, b + 0.9 * drand48());
      if ((center - Vect(4, 0.2, 0)).magnitude() > 0.9 && i < n - 4 &&
          drand48() > 0.5) {
        if (choose_mat < 0.8) {
          list[i++] = new Sphere(
              center, 0.2,
  ...
  list[i++] =
      new Sphere(Vect(-4, 1, 0), 1.0, new Lambertian(Vect(0.4, 0.2, 0.1)));
...
void savePixel(Raytracer *raytracer, Camera *camera, RGBType *pixels,
  ...
  final = Vect(sqrt(final.getVectX()), sqrt(final.getVectY()),
               sqrt(final.getVectZ()));
  pixels[index].r = final.getVectX();
  pixels[index].g = final.getVectY();
  pixels[index].b = final.getVectZ();
  if ((x % width) == 0) {
    double value = (((double)x + double(y) * double(width)) /
